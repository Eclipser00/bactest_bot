...
# ==================== BLOQUE PROTEGIDO - NO MODIFICAR ====================
# Este bloque está estabilizado y no debe ser modificado por el agente.
# Si necesitas cambios aquí, propónlos explícitamente al usuario.
...
class Tres_Medias_Filtered(_BaseLoggedStrategy):
    ...
    Tendencial con filtro de pendiente de SMA3 y ADX>20:
      - Si pendiente(sma3) > +slope_th y ADX>20 -> sólo largos (al cruce sma1>sma2)
      - Si pendiente(sma3) < -slope_th y ADX>20 -> sólo cortos (al cruce sma1<sma2)
    """
...
class SMA_sobre_RSI(_BaseLoggedStrategy):
    ...
    def next(self):
        ...
        # Entradas y salidas (idéntico a tu lógica)
...
class RSI_WMA_TurnFilters(_BaseLoggedStrategy):
    ...
    RSI + giros de WMA con filtros:
      ...
      - Stops: mínimos/máximos previos (lookback=n3).
    """
    ...
    def next(self):
        ...
        # Stops dinámicos si hay posición
...
class Bollinger_Bands(_BaseLoggedStrategy):
    """
    Reversión a la media con Bandas de Bollinger usando CRUCES:
      ...
      - Stop  = último pivote confirmado (máximo/minimo)
      - TP    = banda media
    """
    params = (('n1', 50), ('n2', 2), ('n3', 10), ('size_pct', 0.05))  # n3 no se usa aquí; conservo la firma
    ...
    def next(self):
        ...
        if self.x_close_upper[0] > 0:
            # Close cruzó por ENCIMA de la banda superior -> ruptura superior
            self.close_above_upper = True
            # print(f"[FLAG] {date} Ruptura superior por cruce (close ↑ upper)")

        if self.x_close_lower[0] < 0:
            # Close cruzó por DEBAJO de la banda inferior -> ruptura inferior
            self.close_below_lower = True
            # print(f"[FLAG] {date} Ruptura inferior por cruce (close ↓ lower)")
        ...
        if self.position.size == 0 and self.close_above_upper and self.x_close_upper[0] < 0:
            #print(f"[ENTRY SHORT] {date} Reentrada tras ruptura superior (close ↓ upper).")
        ...
        if self.position.size == 0 and self.close_below_lower and self.x_close_lower[0] > 0:
            #print(f"[ENTRY LONG ] {date} Reentrada tras ruptura inferior (close ↑ lower).")
        ...
        # ---------------------
        if self.position.size < 0:
            # STOP por invalidación (precio supera el último máximo confirmado)
        ...
        # ---------------------
        if self.position.size > 0:
            # STOP por invalidación (precio cae por debajo del último mínimo confirmado)
...
class Bollinger_Ratio(_BaseLoggedStrategy):
    ...
    Requiere:
      datas[0] = base, datas[1] = quote, datas[2] = ratio (para señales).
    """
...
class StrategyMultiAsset(_BaseLoggedStrategy):
    ...
    class _InnerContext:
        ...
        def __getattr__(self, name):
            """
            Delegar métodos que no están en _InnerContext hacia inner_cls.
            Esto permite que métodos auxiliares como _wma_turns_up() funcionen
            ...
            if hasattr(self._inner_cls, name):
                ...
                if callable(method):
                    # Crear un método vinculado que use 'self' (el contexto) como primer argumento
