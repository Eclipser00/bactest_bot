...
from data import safe_name, ensure_dirs         # :contentReference[oaicite:2]{index=2}
...
class Grid:
    ...
    def save(grid_df: pd.DataFrame, strategy_name: str, suffix: str = "") -> str:
        ensure_dirs()
        fname = f"grid/{safe_name(strategy_name)}{('_' + suffix) if suffix else ''}.xlsx"
...
class Heatmaps:
    ...
    def make(grid_df: pd.DataFrame, param_cols: List[str], metric: str, strategy_name: str) -> str:
        ensure_dirs()
        filename = f"heatmaps/{safe_name(strategy_name)}_{safe_name(metric)}.html"
    ...
    def save(metrics_df: pd.DataFrame, strategy_name: str, params: Optional[Dict[str, Any]] = None) -> str:
        ...
        if params:
            for k, v in params.items():
                df[k] = v
        fname = f"stats/{safe_name(strategy_name)}.xlsx"
...
class Trades:
    ...
    def save(trades_df: pd.DataFrame, strategy_name: str, params: Optional[Dict[str, Any]] = None) -> str:
        ...
        if params:
            for k, v in params.items():
                df[k] = v
        fname = f"trades/{safe_name(strategy_name)}.xlsx"
        df.to_excel(fname, index=False)
...
class CSCV:
    ...
    def save(cscv_df: pd.DataFrame, strategy_name: str, suffix: str = "") -> str:
        ensure_dirs()
        df = cscv_df.copy()
        fname = f"cscv/{safe_name(strategy_name)}{('_' + suffix) if suffix else ''}.xlsx"
        df.to_excel(fname, index=False)
...
class Stress:
    ...
    def save(stress_df: pd.DataFrame, strategy_name: str, suffix: str = "") -> str:
        ensure_dirs()
        df = stress_df.copy()
        fname = f"stress/{safe_name(strategy_name)}{('_' + suffix) if suffix else ''}.xlsx"
        df.to_excel(fname, index=False)
...
class WF:
    ...
    def save(wf_df: pd.DataFrame, strategy_name: str, suffix: str = "") -> str:
        ensure_dirs()
        df = wf_df.copy()
        fname = f"wf/{safe_name(strategy_name)}{('_' + suffix) if suffix else ''}.xlsx"
        df.to_excel(fname, index=False)
...
class Plots:
    ...
    def make(ohlcv: pd.DataFrame, trades_df: pd.DataFrame, indicators_df: pd.DataFrame,
        ...
        ensure_dirs()
        if not filename:
            filename = f"plots/{safe_name(strategy_name)}.html"
...
class Results:
    ...
    def build_summary(order_by: Optional[str] = None) -> pd.DataFrame:
        ...
        files = [f for f in os.listdir("stats") if f.lower().endswith(".xlsx")]
        rows = []
        for f in files:
            try:
                df = pd.read_excel(os.path.join("stats", f))
                df["__source__"] = f
                rows.append(df)
            except Exception as e:
        ...
        if show_summary:
            out = "results/summary_stats.xlsx"
