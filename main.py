...
import pandas as pd
import config
from data import DataManager, ensure_dirs, erase_directory, safe_name
...
def main() -> None:
    ...
    # Verificar carpetas DATA_FOLDER_01, DATA_FOLDER_02, DATA_FOLDER_03, etc.
    for i in range(1, 100):  # Máximo 99 carpetas (razonable)
        folder_attr = f"DATA_FOLDER_{i:02d}"
        if hasattr(config, folder_attr):
            folder = getattr(config, folder_attr)
            if folder and os.path.isdir(folder):
                data_folders.append(folder)
                print(f"[MAIN] Carpeta de datos detectada: {folder}")
    ...
    # ------------------------------------------------------------------
    dm = DataManager(data_root=".", tz=getattr(config, "TIMEZONE", "UTC"))
        ...
        for strat_cls, params_by_asset in configured_strategies:
            ...
            if params:
                for k, v in params.items():
                    df_result[k] = v
            fname = f"results/{safe_name(base_name)}.xlsx"
